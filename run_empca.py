...
def R2noise(weights,empcamodel,usemad=True):
    ...
    return 1-(Vnoise/var),var,Vnoise
...
def plot_R2(empcamodels,weights,ptitle,savename,labels=None,nvecs=5,usemad=True,hide=True):
    R2noiseval = R2noise(weights,empcamodels[1],usemad=usemad)[0]
...
if __name__=='__main__':
    ...
    if model.subgroups[0] != False:
        for subgroup in model.subgroups:
            ...
            if verbose:
                print 'PIXEL SPACE'
            empcaname = model.outName('pkl',content = 'empca/empca',subgroup=subgroup,order = model.order,seed = model.seed,cross=model.cross,nvecs=nvecs,mad=usemad)
            ...
            R2noiseval1 = R2noise(w1,m1,usemad=usemad)[0]
            R2noiseval2 = R2noise(w2,m2,usemad=usemad)[0]
            ...
            empcaname = model.outName('pkl',content = 'empca/empca_element',order = model.order,seed = model.seed,cross=model.cross,subgroup=subgroup,nvecs=nvecs,mad=usemad)
            m3,m4,w3,w4 = elem_empca(model,residual,errs,empcaname,nvecs=nvecs,gen=gen,verbose=verbose,deltR2=deltR2,usemad=usemad)
            
            R2noiseval3 = R2noise(w3,m3,usemad=usemad)[0]
            R2noiseval4 = R2noise(w4,m4,usemad=usemad)[0]
        ...
        if verbose:
            print 'PIXEL SPACE'
        empcaname = model.outName('pkl',content = 'empca/empca',order = model.order,seed = model.seed,cross=model.cross,nvecs=nvecs,mad=usemad)
        ...
        empcaname = model.outName('pkl',content = 'empca/empca_element',order = model.order,seed = model.seed,cross=model.cross,nvecs=nvecs,mad=usemad)
        m3,m4,w3,w4 = elem_empca(model,residual,errs,empcaname,nvecs=nvecs,gen=gen,verbose=verbose,deltR2=deltR2,usemad=usemad)
